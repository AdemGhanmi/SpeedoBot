...
class _MainScreenState extends State<MainScreen> with TickerProviderStateMixin {
  ...
  String _currentTitle = 'chat'.tr; // Initial title uses translation
  ...
  final List<Map<String, dynamic>> _menuItems = [
    ...
    {'icon': Icons.record_voice_over, 'label': 'voice_cloning'.tr, 'screen': const VoiceCloningScreen()},
...
Future<void> _fetchCategories() async {
  try {
    final response = await http.get(Uri.parse('https://alert.speedobot.com/api/v1/alert/categories'));
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Failed to load categories: $e'.tr)),
    );
  }
}

Future<void> _sendAlert(Uint8List screenshot, String category, String description) async {
  try {
    var request = http.MultipartRequest(
      'POST',
      Uri.parse('https://alert.speedobot.com/api/v1/alert'),
    );
    request.fields['category'] = category.toLowerCase().replaceAll(' ', '_');
    ...
    final response = await request.send().timeout(const Duration(seconds: 30), onTimeout: () {
      throw Exception('Request timed out'.tr);
    });
    final responseData = await http.Response.fromStream(response);

    print('POST Alert - Status Code: ${response.statusCode}');
    print('POST Alert - Response Body: ${responseData.body}');
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      if (jsonResponse['ok'] == true) {
        ...
        throw Exception('Alert submission failed: ${jsonResponse['message'] ?? 'Unknown error'}');
      }
    } else {
      throw Exception('Failed to send alert: HTTP ${response.statusCode} - ${responseData.body}');
    }
  } catch (e) {
    print('POST Alert - Error: $e');
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Error sending alert: $e'.tr)),
    );
  }
}
 ...
 Future<Uint8List?> _captureScreenshot() async {
  try {
    if (_repaintKey.currentContext == null) {
      print('Screenshot Error: Widget not rendered yet!');
      return null;
    }
    ...
    print('Screenshot Captured - Size: ${screenshot?.length ?? 0} bytes');
    return screenshot;
  } catch (e) {
  ...
  Future<void> _showAlertDialog() async {
    ...
    if (screenshot == null || screenshot.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('image_error'.tr)),
      );
      return;
    }
      ...
      builder: (ctx) {
        ...
        return StatefulBuilder(
          builder: (context, setDialogState) {
            return Dialog(
              backgroundColor: isDark ? const Color(0xFF02111a) : Colors.white,
                    ...
                    children: [
                      ...
                      Container(
                        decoration: BoxDecoration(
                          border: Border.all(color: const Color(0xFF41be8c), width: 2),
                      ...
                      const SizedBox(height: 20),
                      _buildCategoryDropdown(isDark, selectedCategory, _categories, _isLoadingCategories, (val) {
                        setDialogState(() {
                          selectedCategory = val;
                        });
                      }),
                      const SizedBox(height: 20),
                      _buildDescriptionField(isDark, descController),
                      ...
                      Row(
                        ...
                        children: [
                          TextButton(
                            ...
                            child: Text(
                              'cancel'.tr,
                              style: TextStyle(color: const Color(0xFF41be8c)),
                            ),
                          ),
                          const SizedBox(width: 16),
                          ElevatedButton(
                            onPressed: isSubmitting || selectedCategory == null
                                ? null
                                : () async {
                                    final description = descController.text;
                                    if (selectedCategory!.toLowerCase() == 'other' && description.length < 10) {
                                      ScaffoldMessenger.of(context).showSnackBar(
                                        SnackBar(
                                          content: Text('other_description_error'.tr),
                                        ),
                                      );
                                      return;
                                    }
                                    ...
                                    await _sendAlert(screenshot, selectedCategory!, description);
                                    setDialogState(() {
                                      isSubmitting = false;
                                    });
                                    Navigator.pop(ctx);
                            ...
                            style: ElevatedButton.styleFrom(
                              backgroundColor: const Color(0xFF41be8c),
  ...
  Widget _buildCategoryDropdown(
      bool isDark, String? selectedCategory, List<String> categories, bool isLoading, ValueChanged<String?> onChanged) {
    return Card(
      elevation: 2,
      ...
      child: Padding(
        ...
        child: isLoading
            ...
            : DropdownButton<String>(
                ...
                dropdownColor: isDark ? const Color(0xFF02111a) : Colors.white,
                items: categories
  ...
  Widget _buildDescriptionField(bool isDark, TextEditingController controller) {
    return Card(
      elevation: 2,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
      color: isDark ? Colors.white.withOpacity(0.05) : Colors.grey[50],
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: const Icon(Icons.bug_report, color: Color(0xFF41be8c)),
