...
class _VoiceCloningScreenState extends State<VoiceCloningScreen> {
  ...
  void _startGeneration() async {
    ...
    print(
        'VoiceCloningScreen: Starting generation - Prompt: ${_promptController.text}, Language: $_selectedLanguage, Audio: ${_selectedAudio!.path}');
    await _playClickSound();
    ...
    try {
      ...
      if (response['success']) {
        print(
            'VoiceCloningScreen: Generation successful - URL: ${response['url']}');
        setState(() {
          _isGenerating = false;
          _showResult = true;
        ...
        print(
            'VoiceCloningScreen: Generation failed - Error: ${response['error']}');
        setState(() {
          _isGenerating = false;
        });
  ...
  Future<void> _playResponse() async {
    print(
        'VoiceCloningScreen: Playing response audio - URL: $_generatedAudioUrl');
    await _playClickSound();
      ...
      children: [
        ElevatedButton(
          onPressed: _isGenerating
              ? null
              : _pickAudio, // Disable button when generating
              ...
              : Row(
                  ...
                  children: [
                    ...
                    GestureDetector(
                      onTap: () async {
                        print(
                            'VoiceCloningScreen: Removing selected audio: ${_selectedAudio!.path}');
                        await _playClickSound();
