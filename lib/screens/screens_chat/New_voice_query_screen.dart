...
class _VoiceQueryScreenState extends State<VoiceQueryScreen> {
  ...
  final String _fallbackVideoUrl =
    ...
    for (int i = 0; i < _videoUrls.length; i++) {
      if (!_carouselControllers.containsKey(i)) {
        ...
        try {
          ...
          _logger
              .e('Failed to initialize carousel controller for index $i: $e');
        }
      }
    }
    if (mounted) {
  ...
  Future<void> _initializeVideoController(String videoUrl) async {
    ...
    try {
      ...
      try {
        ...
        _videoController =
            VideoPlayerController.networkUrl(Uri.parse(_fallbackVideoUrl));
        await _videoController!.initialize();
        if (mounted) {
          setState(() {
            _isVideoInitialized = true;
        ...
        await _videoController!.setLooping(true);
        _logger
            .i('Fallback video initialized successfully: $_fallbackVideoUrl');
      } catch (fallbackError) {
        _logger.e('Error initializing fallback video: $fallbackError');
        try {
          _logger.i('Trying local asset video as final fallback');
          _videoController =
              VideoPlayerController.asset('assets/fallback_video.mp4');
          await _videoController!.initialize();
          if (mounted) {
            setState(() {
              _isVideoInitialized = true;
  ...
  Future<String> _getTempFilePath() async {
    ...
    final path =
        '${dir.path}/recorded_message_${DateTime.now().millisecondsSinceEpoch}.aac';
    ...
    try {
      ...
      if (path != null) {
        ...
        if (await file.exists()) {
          _logger.i(
              'Recording saved at: $path, size: ${await file.length()} bytes');
          setState(() {
            _isRecording = false;
            _recordedFile = file;
  ...
  Future<void> _playRecording(File? file) async {
    ...
    try {
      if (_isPlaying) {
        ...
        setState(() {
          _isPlaying = false;
          _statusText = _recordedFile != null
              ? 'enregistrement_termine'.tr
              : 'presser_pour_enregistrer'.tr;
        });
      } else {
        await _playClickSound();
        await _player.play(DeviceFileSource(file.path));
        if (file == _responseFile &&
            _videoController != null &&
            _isVideoInitialized) {
          await _videoController!.setLooping(true);
          await _videoController!.play();
          _logger.i(
              'Video started playing in loop during _playRecording for response');
        }
        _logger.i('Playing recording from: ${file.path}');
        setState(() {
          _isPlaying = true;
          _statusText = file == _responseFile
              ? 'lecture_reponse'.tr
              : 'lecture_enregistrement'.tr;
        });
  ...
  Future<void> _generateResponse() async {
    ...
    try {
      ...
      final responseFile =
          await _apiService.downloadAudio(audioUrl, responsePath);
      ...
      if (_videoController != null && _isVideoInitialized) {
        ...
        if (_videoUrls.isNotEmpty) {
          ...
          if (_videoController != null && _isVideoInitialized) {
            ...
            await _videoController!.pause();
            _logger.i(
                'Video reinitialized and paused: ${_videoUrls[_selectedVideoIndex]}');
          }
        }
      }
  ...
  void _scrollToResponseSection() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (_scrollController.hasClients &&
          _responseSectionKey.currentContext != null) {
        final RenderBox renderBox =
            _responseSectionKey.currentContext!.findRenderObject() as RenderBox;
        final position = renderBox.localToGlobal(Offset.zero).dy;
        final scrollPosition =
            position - MediaQuery.of(context).padding.top - 50;
        ...
        _logger
            .w('ScrollController or response section not ready, retrying...');
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (_buildRecordingControls != null)
                      _buildRecordingControls(),
                    const SizedBox(height: 24),
                    Row(
                      children: [
                        // زر الإجراء يأخذ المساحة المتبقية
                        Expanded(
                          child: _buildActionButton(expand: true),
                        ),

                        if (_isProcessingResponse) ...[
                          const SizedBox(width: 12),
                          // زر الإلغاء يأخذ المساحة المتبقية أيضاً (يمكن تقليص flex لو تحب)
                          Expanded(
                            child: GestureDetector(
                              onTap: _cancelGeneration,
                              child: AnimatedContainer(
                                duration: Duration(milliseconds: 300),
                                // لا تحدد عرض ثابت — اتركه يملأ المساحة (double.infinity داخل Expanded)
                    ...
                    if (_responseFile != null && !_isProcessingResponse)
  ...
  Widget _buildActionButton({bool expand = false}) {
    return GestureDetector(
      onTap: (_recordedFile != null && !_isProcessingResponse)
          ? _generateResponse
          : null,
      child: AnimatedContainer(
        duration: Duration(milliseconds: 300),
        width: expand ? double.infinity : 180,
        height: 50,
        alignment: Alignment.center,
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(30),
          boxShadow: [
            BoxShadow(color: textColor.withOpacity(0.5), blurRadius: 12)
          ],
        ),
